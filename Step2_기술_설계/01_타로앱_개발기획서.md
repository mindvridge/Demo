...
Mobile & Desktop:
  ...
  Language: C# 10.0
  UI System: Unity UI Toolkit (Runtime UI) + UGUI (레거시 호환)
  State Management: Singleton Pattern + ScriptableObjects
  Animation: Unity Animation System + DOTween Pro
  Physics: Unity Physics 2D/3D
  AR: Unity AR Foundation (ARKit/ARCore)
  Platform: iOS, Android, Windows, macOS

Web:
  Build: Unity WebGL
  Framework: Next.js 14 (랜딩/관리자 페이지)
...
graph TB
    subgraph "Client Layer"
        iOS[Unity iOS App]
        Android[Unity Android App]
        Web[Unity WebGL / Next.js Web]
        Desktop[Unity Desktop App]
    end
...
#### ✅ 장점
1. **크로스 플랫폼 개발**
   - iOS, Android, Windows, macOS, WebGL 모두 단일 코드베이스로 지원
   - Write Once, Deploy Everywhere

2. **3D 그래픽 및 애니메이션 우위**
   - 타로 카드의 3D 표현 및 물리 기반 인터랙션
   - Shader Graph로 고품질 시각 효과 구현
   - Particle System으로 신비로운 분위기 연출

3. **AR 기능 내장**
   - Unity AR Foundation으로 ARKit/ARCore 통합 지원
   - AR 타로 리딩 기능 구현이 용이

4. **성능 최적화**
   - IL2CPP로 네이티브 수준의 성능
   - Addressables로 효율적인 리소스 관리
   - Object Pooling으로 메모리 최적화
...
#### ⚠️ 주의사항
1. **빌드 크기**
   - Unity 기본 빌드 크기가 큼 (100MB+)
   - Addressables와 App Slicing으로 최적화 필요

2. **웹뷰 제한**
   - Unity 내 웹뷰 지원이 제한적
   - 결제, 웹 콘텐츠는 Native Plugin 필요

3. **개발자 학습 곡선**
   - C# 및 Unity 에디터 숙련 필요
   - React Native 대비 초기 학습 시간 증가
...
#### 1. TarotCard ScriptableObject
```csharp
[CreateAssetMenu(fileName = "Card", menuName = "Tarot/Card")]
public class TarotCardData : ScriptableObject
{
    ...
    public Element element; // Fire, Water, Air, Earth
...
public class CardInteraction : MonoBehaviour
{
    ...
    {
        isRevealed = true;
        // DOTween으로 3D Flip 애니메이션
        transform.DORotate(new Vector3(0, 180, 0), 0.6f)
            .SetEase(Ease.OutBack)
            .OnComplete(() => {
                // 카드 공개 완료 이벤트
                EventManager.TriggerEvent("CardRevealed", cardData);
            });
    }
}
```

#### 3. Physics-Based Card Shuffle
```csharp
public class CardShuffleManager : MonoBehaviour
{
    public GameObject cardPrefab;
    public int cardCount = 78;
    ...
    public void ShuffleCards()
    {
        ...
        {
            GameObject card = Instantiate(cardPrefab, RandomPosition(), RandomRotation());
        ...
        // 3초 후 카드 정렬
        StartCoroutine(ArrangeCardsAfterShuffle(3f));
    }
    ...
    IEnumerator ArrangeCardsAfterShuffle(float delay)
    {
        ...
        for (int i = 0; i < cards.Count; i++)
        {
            ...
            cards[i].transform.DOMove(targetPos, 1f).SetEase(Ease.OutQuad);
            cards[i].transform.DORotate(Vector3.zero, 1f).SetEase(Ease.OutQuad);
        }
    }
}
```

#### 4. API Client (UnityWebRequest)
```csharp
public class APIClient : MonoBehaviour
{
    private const string BASE_URL = "http://localhost:3000/api";

    public IEnumerator GetInterpretation(List<int> cardIds, string question,
        System.Action<InterpretationResponse> onSuccess)
    {
...
public class CardResourceManager : MonoBehaviour
{
    ...
    {
        string address = $"Cards/Card_{cardId}";
...
.reading-screen {
    ...
    align-items: center;
    padding: 20px;
    background-image: url('background_stars.png');
}

.interpretation-text {
    font-size: 16px;
    color: #F3F4F6;
    -unity-font: url('Fonts/Cinzel-Regular.ttf');
    white-space: normal;
}
```
...
<ui:UXML>
    <ui:VisualElement class="reading-screen">
        ...
        <ui:VisualElement class="card-spread">
            <ui:VisualElement name="card-1" class="card-container"/>
            <ui:VisualElement name="card-2" class="card-container"/>
            <ui:VisualElement name="card-3" class="card-container"/>
        </ui:VisualElement>

        <ui:ScrollView class="interpretation-container">
            <ui:Label name="interpretation-text" class="interpretation-text"/>
        </ui:ScrollView>

        <ui:Button text="저장" class="primary-button"/>
    </ui:VisualElement>
</ui:UXML>
```
...
1. **Object Pooling**
   - 카드, 파티클 등 자주 생성/삭제되는 객체 풀링

2. **Addressables**
   - 78장 카드 이미지를 Addressables로 관리
   - 필요할 때만 비동기 로드

3. **IL2CPP 빌드**
   - Mono 대신 IL2CPP로 빌드하여 성능 향상
   - 코드 난독화 및 보안 강화

4. **Shader Optimization**
   - Mobile-optimized shaders 사용
   - Shader variants 최소화

5. **Frame Rate 관리**
   - Application.targetFrameRate = 60 (고사양 기기)
   - Application.targetFrameRate = 30 (저사양 기기)

6. **메모리 관리**
   - Resources.UnloadUnusedAssets() 주기적 호출
   - Garbage Collection 최소화 (struct 활용)
...
#### Sprint 5-6: Unity 클라이언트 기초
- Unity 프로젝트 구조 설정
- UI Toolkit 기반 컴포넌트 라이브러리
- Scene 관리 시스템 및 네비게이션
- 3D 카드 모델 및 셰이더 구현
- 물리 기반 카드 인터랙션

#### Sprint 7-8: 리딩 시스템
- 스프레드 시스템 구현 (ScriptableObjects)
- Unity Physics 기반 카드 셔플 시뮬레이션
- DOTween을 활용한 카드 선택 애니메이션
- AI 해석 통합 (REST API 연동)
- 리딩 히스토리 (로컬 저장 + 클라우드 동기화)

#### Sprint 9-10: 개인화 및 결제
- 개인화 엔진 구현
- 구독 시스템 (Unity IAP)
- iOS/Android 인앱 결제 통합 (Unity IAP + StoreKit/Google Play Billing)
- 푸시 알림 (Firebase Cloud Messaging)

#### Sprint 11-12: 최적화 및 출시
- Unity Profiler를 통한 성능 최적화
- 모바일 배터리 최적화 (60fps → 30fps 옵션)
- 빌드 크기 최적화 (Addressables 적용)
- 보안 점검 (IL2CPP 코드 난독화)
